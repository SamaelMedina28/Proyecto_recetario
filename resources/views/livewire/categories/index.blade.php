<div class="max-w-4xl mx-auto sm:px-4 py-6">
    <div class="mb-6 flex justify-between items-center">
        ...
        <button
            class="bg-emerald-500 hover:bg-emerald-600 text-white font-medium py-2 px-4 rounded-lg transition-all duration-200 transform hover:scale-[1.02] shadow-md hover:shadow-lg">
            {{ __('Add New') }}
        </button>
    </div>
    ...
    <ul class="space-y-3">
        @foreach ($categories as $category)
            <li class="group">
                <div
                    class="flex justify-between items-center bg-white dark:bg-zinc-800 rounded-xl shadow-sm hover:shadow-md py-4 pl-4 pr-2 sm:p-4 transition-all duration-300 border border-gray-100 dark:border-zinc-700 group-hover:border-gray-200 dark:group-hover:border-zinc-600">
                    <span class="text-lg font-medium text-gray-700 dark:text-gray-200">{{ $category->name }}</span>
                    <div class="flex gap-3">
                        <button
                            class="bg-blue-500 hover:bg-blue-600 text-white font-medium py-2 px-2 sm:px-4 rounded-full sm:rounded-lg transition-all duration-200 transform hover:scale-[1.03] shadow hover:shadow-md">
                        ...
                        <button
                            class="bg-red-500 hover:bg-red-600 text-white font-medium py-2 px-2 sm:px-4 rounded-full sm:rounded-lg transition-all duration-200 transform hover:scale-[1.03] shadow hover:shadow-md"
                            wire:click="delete({{ $category->id }})"
                            wire:confirm="Are you sure you want to delete this category?" wire:loading.attr="disabled">
    ...
    <div x-data="{
        ...
        clearTimeout(timer);
        timer = setTimeout(() => { showModal = false }, 2500);
    "
